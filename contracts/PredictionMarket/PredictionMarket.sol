...
contract PredictionMarket is Ownable {
    ...
    constructor(
        ...
        if (_s_ethCollateral < 1e8) {
            revert RewardsTooSmall(_liquidityProvider, _s_ethCollateral);
        }
        if (
            LockingContract.checkUserDeposit(_liquidityProvider) <
            _s_ethCollateral
        ) {
            revert DepositTooSmall(_liquidityProvider);
        }
        ...
        s_ethCollateral = _s_ethCollateral;

        /// Checkpoint 3 ////
        uint256 initialTokenAmount = (_s_ethCollateral * PRECISION) /
            _initialTokenValue;
        ...
        uint256 initialYesAmountLocked = (initialTokenAmount *
            _initialYesProbability *
            _percentageToLock *
            2) / 10000;
        uint256 initialNoAmountLocked = (initialTokenAmount *
            (100 - _initialYesProbability) *
            _percentageToLock *
            2) / 10000;

        LockingContract.transferWHbar(
            _liquidityProvider,
            address(this),
            _s_ethCollateral
        );
    ...
    function addLiquidity(uint256 _s_ethCollateral) external onlyOwner {
        if (LockingContract.checkUserDeposit(msg.sender) < _s_ethCollateral) {
        ...
        uint256 tokensAmount = (_s_ethCollateral * PRECISION) /
            i_initialTokenValue;
        ...
        emit LiquidityAdded(msg.sender, _s_ethCollateral, tokensAmount);
    }

    function removeLiquidity(uint256 _ethToWithdraw) external onlyOwner {
        //// Checkpoint 4 ////
        uint256 amountTokenToBurn = (_ethToWithdraw / i_initialTokenValue) *
            PRECISION;
        ...
        LockingContract.receiveWHbar(msg.sender, _ethToWithdraw);
        ...
        returns (uint256 ethRedeemed)
    {
        ...
        if (contractWinningTokens > 0) {
            ethRedeemed =
                (contractWinningTokens * i_initialTokenValue) /
                PRECISION;
        ...
        notOwner
    {
        ...
        PredictionMarketToken optionToken = _outcome == Outcome.YES
            ? i_yesToken
            : i_noToken;
        ...
        notOwner
    {
        ...
        bool success = optionToken.transferFrom(
            msg.sender,
            address(this),
            _tradingAmount
        );
    ...
    function _calculatePriceInEth(
        ...
        uint256 currentOtherTokenSold = totalTokenSupply -
            currentOtherTokenReserve;

        uint256 totalTokensSoldBefore = currentTokenSoldBefore +
            currentOtherTokenSold;
        uint256 probabilityBefore = _calculateProbability(
            currentTokenSoldBefore,
        ...
        uint256 currentTokenReserveAfter = _isSelling
            ...
            : currentTokenReserve - _tradingAmount;
        uint256 currentTokenSoldAfter = totalTokenSupply -
            currentTokenReserveAfter;

        uint256 totalTokensSoldAfter = _isSelling
            ? totalTokensSoldBefore - _tradingAmount
            : totalTokensSoldBefore + _tradingAmount;

        uint256 probabilityAfter = _calculateProbability(
            currentTokenSoldAfter,
            totalTokensSoldAfter
        );

        /// Compute final price
        uint256 probabilityAvg = (probabilityBefore + probabilityAfter) / 2;
        return
            (i_initialTokenValue * probabilityAvg * _tradingAmount) /
            (PRECISION * PRECISION);
    }
